...
Nwavelets = 50 # ~8000 needed to get Tmax=3 hrs with equal dw, wmax=10*wm
...
if optimize_panel_areas:# {{{
    if verbose: print 'Optimizing panel areas...'
    #print '  before:',w0
...
for irand in range(Nrealizations):
    ...
    if plot_overlay:
        t2= np.linspace(2*Tmax,4*Tmax,Nt) # DEBUG
        t3= np.linspace(4*Tmax,6*Tmax,Nt) # DEBUG
        Z2 = np.zeros((Nt))
        Z3 = np.zeros((Nt))
